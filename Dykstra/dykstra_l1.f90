program dykstra_solution_l1
    ...
    ! Scalar arguments
    integer(int32)    :: n, i, j, k, iter, max_iter, n_real, allocerr, m_small, halfn, ny, nz, nextnnyi
    ...
    n = n_real * (n_real - 1)
    halfn = n / 2
    m_small = (n_real * (n_real - 1)) / 2 * (n_real - 2) + n

    !Allocate 
    allocate(winv(n), x(n), aypc(n), a(halfn), weight(halfn), &
    ...
    winv(1 : halfn) =  gamma * 1.0d0 
    winv(halfn + 1 : n) = gamma * 1.0d0 

    ! Initial value of x
    x(1 : halfn) = 0.0d0 
    x(halfn + 1 : n) = - winv(halfn + 1 : n) 
...
100 continue
    ...
    do i = 1, halfn
       ...
       if (adotx .gt. weight(i) * a(i)) then
            ...
            theta = (adotx - weight(i) * a(i)) / (winv(i) * weight(i) ** 2 + winv(i + halfn))
...
contains
    ...
    subroutine write_parameters(file, n_iter, time, stp, max_inf, cmpl, optm, gap, linear_of, gamma)
        ...
        write(unit_number, '(A10,    6X,    I8,     3X,     1P,F15.4,    8X,     1P,E8.1,     12X, &
